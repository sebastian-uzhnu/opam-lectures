---
sidebar_label: Visual Studio IDE
sidebar_position: 8
---
...
* **Підказки й автодоповнення (IntelliSense)**: середовище пропонує доступні методи,
  показує їх параметри й одразу підкреслює помилки — ще до компіляції.
...
:::tip[Яку версію брати студенту]
**Visual Studio Community** — безкоштовна для навчання, індивідуальних розробників
і відкритих проєктів. За можливостями вона практично не відрізняється від
Professional: усе, що потрібно в цьому курсі, у ній є.

Під час установки достатньо обрати одне робоче навантаження —
**«Розробка класичних застосунків .NET»**. Решту можна доставити пізніше через
Visual Studio Installer, не перевстановлюючи середовище.
:::

:::note[Якщо у вас не Windows]
Visual Studio існує лише для Windows (версію для macOS припинено 2024 року).
На macOS і Linux використовують:

- **Visual Studio Code** + розширення C# Dev Kit — безкоштовно, працює всюди;
- **JetBrains Rider** — платний, але безкоштовний за студентською ліцензією.

Мова, компілятор і .NET на всіх платформах однакові — відрізняється лише
середовище. Код із лекцій працюватиме без змін.
:::
