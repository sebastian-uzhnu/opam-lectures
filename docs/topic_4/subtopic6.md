...
:::note
Функція може викликати саму себе — це називається **рекурсією**, і їй присвячено
[окремий підрозділ](./subtopic10.md).
:::

## Висновок

- **Функція (метод)** — іменований блок коду, який виконує одну задачу
  й може бути викликаний скільки завгодно разів.
- Оголошення містить **тип повернення**, **ім'я** і **список параметрів**;
  `void` означає, що функція нічого не повертає.
- **`return`** повертає значення й одразу завершує виконання функції.
- **Перевантаження** дозволяє мати кілька функцій з однаковим іменем,
  якщо вони різняться списком параметрів.
- Розбиття програми на функції — головний засіб боротьби зі складністю:
  кожну підзадачу можна написати й перевірити окремо.
