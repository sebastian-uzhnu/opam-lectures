...
:::tip[Перевірка на «добрий опис»]
Опис коміту має продовжувати фразу: «Цей коміт **…**».

- ✅ `fix: виправлено ділення на нуль при порожньому списку` —
  «цей коміт виправляє ділення на нуль»;
- ❌ `update` — «цей коміт оновлює»… що?
- ❌ `works now` — «цей коміт працює тепер»?

Пишіть про **що** змінилося, а не про те, що ви робили. Читач історії шукає
конкретну зміну, а не звіт про ваш день.
:::

:::warning[Історія пишеться для майбутнього вас]
Через три місяці програма зламається, і ви шукатимете, коли саме зникло
правильне обчислення. Історія з рядків `update`, `fix`, `final`, `final2`
не допоможе взагалі — доведеться переглядати кожен коміт вручну.

Добрі повідомлення коштують п'ятнадцяти секунд під час коміту й економлять
години під час пошуку помилки. Це найвигідніша звичка, яку можна засвоїти
на цьому курсі.
:::
