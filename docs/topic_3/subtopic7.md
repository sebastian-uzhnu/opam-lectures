...
:::note[Не плутайте з логічними]
`&&` і `&` виглядають схоже, але це різні операції:

| | Логічні `&&`, `\|\|` | Побітові `&`, `\|` |
|---|---|---|
| Операнди | Тільки `bool` | Цілі числа |
| Результат | `bool` | Ціле число |
| Обчислення | Скорочене | Завжди обидва операнди |

```csharp
bool x = true && false;   // false — логічна операція
int  y = 5 & 3;           // 1 — побітова операція над бітами
```
:::
