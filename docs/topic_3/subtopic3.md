---
sidebar_label: Ієрархія типів
sidebar_position: 4
---

# Ієрархія типів і операція `new`
...
:::tip[Перевірити це можна прямо зараз]
Оскільки навіть `int` успадковує `Object`, ці методи працюють для звичайного числа:
...
Console.WriteLine(number.Equals(42));        // True
```

Саме тому `Console.WriteLine(someObject)` виводить хоч щось для будь-якого типу —
метод `ToString()` є в усіх без винятку.
:::
...
:::note[`new` для значимих типів]
```csharp
int a = new int();    // 0 — працює, але так не пишуть
int b = 0;            // ✅ звичайний спосіб
int c = default;      // ✅ те саме
```

`new` для значимого типу не виділяє пам'ять у купі — він лише ініціалізує
значення за замовчуванням. Результат той самий, що й у звичайного присвоєння,
тому цей запис майже не використовують.
:::
