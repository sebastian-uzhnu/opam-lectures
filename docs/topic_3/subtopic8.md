...
:::warning[Як виглядає StackOverflowException]
Найчастіша причина — рекурсія без умови виходу:

```csharp
void Infinite()
{
    Infinite();     // кожен виклик додає кадр у стек
}
```

Кожен виклик методу займає місце у стеку, а стек має обмежений розмір
(близько 1 МБ). За кілька тисяч вкладених викликів він вичерпається.

Особливість цього винятку: його **неможливо перехопити** через `try/catch` —
програма завершується примусово. Тому рекурсія завжди має мати умову виходу
(детально — у темі 4).
:::

:::tip[Практичний висновок]
У цьому курсі ви не керуєте пам'яттю вручну — це робить середовище. Розуміти
стек і купу потрібно для іншого: щоб не дивуватися, чому зміна одного масиву
змінила «інший» масив (насправді той самий), і чому глибока рекурсія падає.
Обидва питання розібрані в наступному підрозділі.
:::
