...
Різниця між «нуль» і «значення невідоме» принципова. Якщо в таблиці оцінок
поле порожнє, `0` означав би «двійка», а `null` — «студент ще не складав».
Плутати їх у базі даних — джерело неправильної статистики.

Коротший запис для значення за замовчуванням — оператор `??`:

```csharp
int? mark = null;
int result = mark ?? 0;      // якщо null, узяти 0
```
:::
...
:::warning[Boxing коштує продуктивності]
Упакування створює новий об'єкт у купі й копіює туди значення. В одиничному
випадку це непомітно, але в циклі на мільйон ітерацій — це мільйон зайвих
об'єктів, які потім доведеться прибирати збирачу сміття.

Саме тому в сучасному C# використовують узагальнення (`List<int>`), а не
колекції на базі `object` — вони працюють зі значимими типами без упакування.
Детально — у темі 10.
:::
